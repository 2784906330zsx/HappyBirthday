...
fetch('date.json')
  ...
  .then((data) => {
    const endDate = `${data.year}-${data.month}-${data.day} ${data.hour}:${data.minute}:${data.second}`;
    if (new Date(endDate) - new Date() > 20000) {
  ...
  fetch('config.json')
    ...
    .then((config) => {
      ...
      replyBtn.addEventListener('click', () => {
        window.open(expressUrl); // 使用从config.json中获取的URL
      });
    })
    .catch((error) => {
      console.error('读取 config.json 时出错:', error);
    });
};
